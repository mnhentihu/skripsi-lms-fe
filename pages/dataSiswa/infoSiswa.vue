...
export default {
  ...
  mounted() {
    Promise.all([
      this.$axios.get(`api/image/${this.$route.query.idSiswa}`),
      this.$axios.get(`api/auth/user/${this.$route.query.idSiswa}`),
      this.$axios.get(`api/exam/${this.$route.query.idSiswa}`),
    ]).then((res) => {
  ...
  methods: {
    async updateImage() {
      ...
      try {
        await this.$axios
          .post(`${this.API_URL}/api/updateimage/${this.userData.id}`,
            formData,
            config
          )
          .then((res) => {
